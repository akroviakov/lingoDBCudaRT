...
template<typename T>
T* loadColumn(string dataSetPath, string dir, string col_name, int num_entries) {
  T* h_col = new T[num_entries];
  string filename = dataSetPath + dir + lookup(col_name);
...
template<typename T>
int storeColumn(string dataSetPath, string dir, string col_name, int num_entries, int* h_col) {
  string filename = dataSetPath + dir + lookup(col_name);
